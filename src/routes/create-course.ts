...
export const creatCoursesRoute:FastifyPluginAsyncZod = async (server) => {
    server.post('/courses', {
        schema: {
            ...
            description: 'Essa rota recebe um título e uma descrição e cria um curso no banco de dados',
            body: z.object({
            ...
            response: {
                ...
                400: z.object({ message: z.string()}).describe('Parametro obrigatorio não informado')
            }
        }
    }, async (request, reply) => {
